...
class Openai(BaseLLM):
    ...
    def __init__(self, model_config: ModelConfig) -> None:
        ...
        self.modalities = [
            m for m in self.config.modalities if m in {"text", "audio"}
        ] or NOT_GIVEN  # type:ignore
        self.audio = (self.config.audio if
                      (self.modalities and self.config.audio) else NOT_GIVEN)
    ...
    def _build_messages(self, request: ModelRequest) -> list:
        ...
        } if not request.resources else self.__build_multi_messages(request))

        messages.append(user_content)

        return messages

    def _tool_call_request_precheck(self,
                                    message: ChatCompletionMessage) -> bool:
        """
        工具调用请求预检
        """
    ...
    async def _ask_sync(
        ...
        try:
            ...
            logger.debug(
                f"OpenAI response: id={response.id}, choices={response.choices}, usage={response.usage}"
            )
            ...
            if (hasattr(message, "reasoning_content")  # type:ignore
            ...
            if response.choices[
                    0].finish_reason == "tool_calls" and self._tool_call_request_precheck(
    ...
    async def _ask_stream(
        ...
        try:
            ...
            async for chunk in response:
                ...
                logger.debug(
                    f"OpenAI response: id={chunk.id}, choices={chunk.choices}, usage={chunk.usage}"
                )
                ...
                if (hasattr(delta, "reasoning_content")
                    ...
                    reasoning_content = chunk.choices[
                        0].delta.reasoning_content  # type:ignore
                    stream_completions.chunk = (reasoning_content
                                                if is_insert_think_label else
                                                "<think>" + reasoning_content)
                    yield stream_completions
                    is_insert_think_label = True

                elif answer_content:
                    stream_completions.chunk = (answer_content if
                                                not is_insert_think_label else
                                                "</think>" + answer_content)
                    yield stream_completions
                    is_insert_think_label = False
            ...
            if function_id:
                function_return = await function_call_handler(
                    function_name, json.loads(function_arguments))
                ...
                async for chunk in self._ask_stream(messages, tools,
                                                    response_format,
                                                    total_tokens):
                    yield chunk
                return
            ...
            if audio_string:
                ...
                stream_completions.resources = [
                    Resource(type="audio", raw=wav_io)
                ]
                yield stream_completions
    ...
    async def ask(
        ...
        *,
        ...
        if request.format == "json" and request.json_schema:
            response_format = ResponseFormatJSONSchema(
                ...
                json_schema=JSONSchema(**
                                       request.json_schema.model_json_schema(),
                                       strict=True),
            )
        else:
        ...
        if stream:
            return self._ask_stream(messages, tools,
                                    response_format)  # type:ignore

        return await self._ask_sync(messages, tools,
                                    response_format)  # type:ignore
