...
__plugin_meta__ = PluginMetadata(
    name="muicebot_plugin_moechat",
    description="是优化群组聊天的插件",
    usage="/敲 @someone",
)
...
command_poke = on_alconna(
    Alconna(
        ...
        "敲",
        Args["target", At],
        meta=CommandMeta("敲某人", usage="/敲 @沐雪"),
    ),
    priority=50,
    block=True,
)
...
command_hug = on_alconna(
    Alconna(
        ...
        "抱",
        Args["target", At],
        meta=CommandMeta("抱某人", usage="/抱 @沐雪"),
    ),
    priority=50,
    block=True,
)
...
command_stick = on_alconna(
    Alconna(
        ...
        "贴",
        Args["target", At],
        meta=CommandMeta("和某人贴贴", usage="/贴 @沐雪"),
    ),
    priority=50,
    block=True,
)
...
async def poke(event: Event, target: Match[At] = AlconnaMatch("target")):
    ...
    message = (UniMessage(f"{user_name}敲了") +
               At(flag="user", target=target.result.target) + UniMessage("!"))
...
async def hug(event: Event, target: Match[At] = AlconnaMatch("target")):
    ...
    message = (UniMessage(f"{user_name}抱了抱") +
               At(flag="user", target=target.result.target) + UniMessage("!"))
...
async def stick(event: Event, target: Match[At] = AlconnaMatch("target")):
    ...
    message = (UniMessage(f"{user_name}和") +
               At(flag="user", target=target.result.target) +
               UniMessage("贴贴~"))
