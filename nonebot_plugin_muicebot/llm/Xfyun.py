...
class Xfyun(BasicModel):
    ...
    def __create_url(self):
        ...
        # 进行hmac-sha256进行加密
        signature_sha = hmac.new(self.api_secret.encode('utf-8'), signature_origin.encode('utf-8'), # type: ignore
    ...
    def __generate_history(self, history: list):
        ...
        self.history.append({"role": 'user', "content": self.__generate_system_prompt(history[0][0]) + history[0][1]})
        for item in history:
    ...
    def ask(self, prompt: str, history: list) -> str:
        ...
        if not self.is_history:
            self.history.append({"role": 'user', "content": self.__generate_system_prompt(prompt) + prompt})
        else:
